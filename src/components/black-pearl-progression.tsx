...
const steps = [
  ...
  {title:"Explore a card-based workspace",src:"black-pearl-hifi-cards.png",text:"A high-fidelity exploration groups the task inside a panel, keeps the map compact, and turns tools into individual launch cards. The repeated containers compete for attention."},
  {title:"Reduce the visual weight",src:"black-pearl-hifi-refined.png",text:"Tool cards become quiet rows and the map grows alongside the task. The task still has a surrounding panel and right-aligned actions; the final screen removes that frame and brings the actions back under the text."},
...
export function BlackPearlProgression(){
 ...
 return <section className={styles.carousel} aria-label="Black Pearl design progression" aria-roledescription="carousel" onKeyDown={e=>{if(e.key==="ArrowRight"){e.preventDefault();move(1);}if(e.key==="ArrowLeft"){e.preventDefault();move(-1);}}}>
  <div className={styles.stage} onTouchStart={e=>{start.current=e.touches[0].clientX;}} onTouchEnd={e=>{if(start.current!==null){const delta=e.changedTouches[0].clientX-start.current;if(Math.abs(delta)>60)move(delta<0?1:-1);}start.current=null;}}>
   <a href={`/artifacts/redesigned/${step.src}`} target="_blank" rel="noreferrer" aria-label={`Open full-size: ${step.title}`}><Image key={step.src} src={`/artifacts/redesigned/${step.src}`} alt={step.title} width={1440} height={step.src.endsWith(".png")?1000:820} sizes="(max-width:767px) calc(100vw - 40px), 900px" /></a>
  </div>
