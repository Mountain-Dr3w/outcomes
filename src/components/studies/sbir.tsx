    ...
    <div className={styles.canvas}>
      <div className={styles.app}>
        ...
        <dialog ref={details} className={styles.dialog} aria-labelledby="topic-detail-title">
          {selectedTopic && <><div className={styles.dialogHeader}><span>Topic details</span><button onClick={() => details.current?.close()} aria-label="Close topic"><X size={23} /></button></div><div className={styles.detailBody}><div className={styles.topicMeta}><span>{selectedTopic.agency} · Phase {selectedTopic.phase}</span><span>{selectedTopic.deadline}</span></div><h2 id="topic-detail-title">{selectedTopic.title}</h2><p>{selectedTopic.summary}</p><dl><div><dt>Topic reference</dt><dd>{selectedTopic.id}</dd></div><div><dt>Funding amount</dt><dd>Not provided in this sample</dd></div><div><dt>Source</dt><dd>{selectedTopic.source}</dd></div></dl><a className={styles.primaryButton} href={selectedTopic.sourceUrl} target="_blank" rel="noreferrer">Visit source website<ArrowUpRight size={18} /></a></div></>}
        </dialog>
