...
export function CrewStudy() {
  ...
  return <div className={styles.app}>
        ...
        {tab!=="Tools"&&<div className={styles.primaryColumn}>
          {tab==="Overview"&&<>
            <section className={styles.nextTask} aria-labelledby="next-task-title">
              ...
              {nextRequest?<><div className={styles.nextTaskTitle}><h3>{nextRequest.title}</h3><span>Due {nextRequest.due} Z</span></div><p>{nextRequest.next}</p><div className={styles.requestActions}><button className={styles.primary} onClick={()=>setSelected(requests.indexOf(nextRequest))}>Open request<ArrowRight size={16} aria-hidden="true"/></button><button className={styles.secondary} onClick={()=>setTab("Requests")}>All requests</button></div></>:<><h3>You’re caught up.</h3><p>Review the request list for work waiting on input or an owner.</p></>}
    ...
    <dialog ref={details} className={styles.detailDialog} onClose={()=>setSelected(null)} aria-labelledby="request-title">{request&&<><div className={styles.dialogTop}><span>{request.id} / {request.type}</span><button aria-label="Close request" onClick={()=>setSelected(null)}><X size={22}/></button></div><h2 id="request-title">{request.title}</h2><p className={styles.description}>{request.description}</p><dl className={styles.properties}><div><dt>Owner</dt><dd>{owner(request)}</dd></div><div><dt>Due today</dt><dd>{request.due} Z</dd></div><div><dt>Status</dt><dd>{status(request)}</dd></div></dl><div className={styles.next}><span>NEXT ACTION</span><p>{complete.includes(request.id)?"Assessment complete. Your update is ready for the next crew.":request.next}</p></div><div className={styles.detailActions}>{owner(request)==="Unassigned"?<button className={styles.primary} onClick={()=>setClaimed(true)}>Assign to me</button>:<button className={styles.primary} onClick={()=>setTool(request.tool)}>Open {request.tool.toLowerCase()}<ArrowUpRight size={16} aria-hidden="true"/></button>}<button className={styles.secondary} onClick={()=>setComplete(complete.includes(request.id)?complete.filter(id=>id!==request.id):[...complete,request.id])}>{complete.includes(request.id)?"Reopen request":"Mark complete"}</button></div><div className={styles.references}><h3>Related material</h3>{["Previous shift’s assessment","Collection notes & coverage"].map(t=><button key={t} onClick={()=>setTool(t)}><FileText size={16} aria-hidden="true"/>{t}<ArrowUpRight size={15} aria-hidden="true"/></button>)}</div></>}</dialog>
